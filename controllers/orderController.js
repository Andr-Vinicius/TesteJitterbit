...
const getById = async (req, res) => {
  try {
    ...
    const [orders] = await db.query("SELECT * FROM `Order` WHERE orderId = ?", [id]);
...
const insert = async (req, res) => {
  ...
  try {
    ...
    if (!value) return res.status(400).json({ mensagem: 'Campo "value" é obrigatório' });
    ...
    if (Array.isArray(items) && items.length > 0) {
      const itemValues = items.map(i => [orderId, i.productId, i.quantity, i.price]);
...
// Atualização do valor de um pedido existente
...
const deleteById = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query("DELETE FROM `Order` WHERE orderId = ?", [id]);
    ...
    await connection.commit();
    res.json({ mensagem: "Pedido e itens removidos" });
  } catch (error) {
    await connection.rollback();
    res.status(500).json({ erro: "Erro ao excluir pedido", detalhe: error.message });
  } finally {
    connection.release();
  }
};
